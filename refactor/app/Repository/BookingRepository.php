...
//            $jobs['total'] = $totaljobs;
    ...
    public function store($user, $data)
    {
        ...
        if ($data['immediate'] == 'no') {
            $this->validateDueDateAndTime($data);
            $this->validateCustomerOptions($data);
        } else {
            $data['immediate'] = 'yes';
        ...
        $this->setGenderAndCertifiedOptions($data);
        ...
        $response = [
            ...
            'job_for' => $this->getJobForOptions($job),
        ];
    ...
    protected function validationErrorResponse($fieldName, $message)
    {
        return [
            'status' => 'fail',
            'message' => $message,
    ...
    protected function validateDueDateAndTime($data)
    {
        if (empty($data['due_date']) || empty($data['due_time'])) {
            $fieldName = empty($data['due_date']) ? "due_date" : "due_time";
            $this->validationErrorResponse($fieldName, "Du måste fylla in alla fält");
        }
    }

    protected function validateCustomerOptions($data)
    {
    ...
    protected function setGenderAndCertifiedOptions(&$data)
    {
    ...
    protected function setJobType(&$data, $consumerType)
    {
        switch ($consumerType) {
            case 'rwsconsumer':
                $data['job_type'] = 'rws';
    ...
    protected function getJobForOptions($job)
    {
        $jobFor = [];
        if ($job->gender != null) {
